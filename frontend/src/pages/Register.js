...
export default function Register() {
  ...
  return (
    <div className="min-h-screen bg-[#F9FAFB] flex" data-testid="register-page">
      ...
      <div className="flex-1 flex items-center justify-center p-6 md:p-12">
        <div className="w-full max-w-sm animate-fade-in-up">
          <h2 className="font-['Outfit'] text-3xl font-bold tracking-tight mb-2 text-[#1E293B]">Create Account</h2>
          <p className="text-sm text-[#64748B] font-mono mb-8">Start your learning journey today.</p>

          {error && (
            <div className="rounded-lg border border-red-200 bg-red-50 p-3 mb-6 text-sm text-red-600 font-mono" data-testid="register-error">
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label className="text-xs font-medium text-[#64748B] font-mono mb-2 block">Full Name</label>
              <input type="text" value={name} onChange={(e) => setName(e.target.value)}
                ...
                placeholder="Your name" required data-testid="register-name-input" />
            </div>
            <div>
          ...
          <p className="text-sm text-[#64748B] font-mono mt-6 text-center">
            ...
            <Link to="/login" className="text-[#1E3A8A] font-semibold hover:underline" data-testid="register-login-link">Sign in</Link>
          </p>
        </div>
      </div>
    </div>
